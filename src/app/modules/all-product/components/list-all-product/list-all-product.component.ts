...
import { ServiceAllProductService } from '../../services/service-all-product.service';
...
export class ListAllProductComponent implements OnInit {
  ...
  getListAllProduct(etiqueta?: string) {
      ...
      .subscribe((res) => {
        ...
        if (etiqueta) {
          this.listProductos = this.listProductos.filter((producto: any) =>
            producto.label.toLowerCase().includes(etiqueta.toLowerCase())
          );
        }
